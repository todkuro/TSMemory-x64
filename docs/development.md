...
- **`<tw>` は百分率ではなく倍率。**`aviutl2.txt` の例が `<tw0.8>` に
  なっている。中型 (MSZ) を `<tw50>` と書いていた為、**50 倍に
  引き伸ばされた文字が横一線に潰れて画面を横切っていた**
  (実機で「楕円のような線」として現れた)。正しくは `<tw0.5>`。
  `obj.w` も巨大になるので、スクリプト側の位置合わせも壊れる。
