...
**フィルタ効果の項目名は本体が持っている物で、こちらからは
確かめる術が無い。**`effect.conf` の `[OldScript.縁取り] color=縁色`
と `aviutl2.exe` の文字列 (「縁取り」「縁色」「パターン画像」) から
組み立てている。名前が違えばエラーになるだけなので `pcall` で包み、
**失敗しても字幕そのものは出る**ようにしてある。効いたかどうかは
`obj.w` が広がるかで判り、デバッグ表示に出している。
