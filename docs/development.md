...
(`aviutl2.txt`「色の変更(文字色,影縁色)」) で、
放送の黒い箱そのものは作れない。テキストプリセット側で文字装飾を
縁取りにして代用する形になる。

その上で、**実際の放送では背景色が出せていない**。
背景の指定 (`90 51`) は上の色配列選択と組で使われており、
実測した番組は全て CLUT 4 を選んでいた。CLUT 1 以上の 112 色は
ARIB STD-B24 の拡張パレットで、まだ実装していない。
判らない色は指定を出さず、プリセット側の色に任せている
(`AribColorIsKnown()`)。**判っているのは CLUT 0 の 16 色だけ。**

`[Caption] UseBackColor` は実装済みで単体試験も通るが、
上記の為に実放送では効かない。

> **色配列 4 の 1 番は「半透明の黒」。**
> TVCaptionMod2 の `Caption_src/ColorDef.h` にある ARIB の 128 色表では
> 索引 65 が `(0, 0, 0, α128)` になっている。放送の字幕の背景は
> **半透明の黒い箱**で、TVCaptionMod2 に「背景の不透明度」の設定が
> あるのはこのアルファを触る為。
> 索引の作り方 (`色配列 << 4 | 番号`) も同実装と一致していた。
...
- **画面上の位置は再現していない。**座標は読めているので
  (`AribItem::A` / `B` にドットで入る)、あとは AviUtl2 の
  `<p x,y>` か オブジェクトの座標に写すだけだが、
  `<p>` の基準座標が `aviutl2.txt` に書かれておらず、
  実機で当ててみないと決められない。
