...
- **文字の表示位置は再現しません。** 放送の絶対位置指定は改行として
  現れます。画面のどこに出すかはオブジェクト設定で調整してください。
- **文字スーパー**(緊急放送等の別系統の字幕) は取り込みません。
- **背景の箱は作れません。** AviUtl2 のテキストには背景色が無く、
  指定できるのは影・縁色だけです。プリセット側で文字装飾を
  「縁取り文字」にすると近い見た目になります。
  なお実際の放送では背景色が ARIB の拡張パレットで指定されている事が多く、
  その色は未対応なので現状ではほとんどの番組で色が付きません。
