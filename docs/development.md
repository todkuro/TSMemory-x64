...
> 内容、隠し設定などを記録してあります。
> 導入方法や使い方は [../README.md](../README.md)、
> 字幕の設定と手順は [README_Subtitles.md](README_Subtitles.md) を
> 参照してください。
...
> **ここは「渡せるか」を調べた記録。**実際の作りでは、外字の大半は
> 対応表で本物の文字に置き換えており、フォントを使うのは残りだけ。
> 経緯は「外字のフォントは初期化の中でしか登録できない」を参照。
...
> **ただし `register_font_collection()` は初期化の中でしか呼べない。**
> 取り込みの時に呼ぶと本体ごと落ちる。字形は取り込むまで判らない為、
> 受け取った字形をファイルに貯めておき、**次の起動の初期化で**渡している。
> 「外字のフォントは初期化の中でしか登録できない」を参照。
...
| ARIB | AviUtl2 |
| --- | --- |
| 文字色・背景色 (COL) | `<#ffffff>` `<#ffffff,000000>` (2 つ目は影縁色) |
| 文字サイズ (SSZ / MSZ / NSZ / SZX) | `<s72>` `<s*0.5>` `<tw0.5>` |
| ルビ (小型で 1 行上) | `</>漢字<!>ふりがな</>` |
| 太字・斜体 (MDF) | `<@+B>` `<@+I>` |
| 外字 (DRCS) | 大半は**本物の文字**に置き換える。残りは `<@組み立てたフォント名>` |
| **書体の一括変更** | **`<$プリセット名>`** |

**表示位置は制御文字では出さない。**`<p x,y>` は 1 文字ずつ置く
指定で、使うとテキストとして編集できなくなる。行ごとに
テキストオブジェクトを分け、**オブジェクトの X / Y** に入れている。
...
| `drcs_replace.cpp` | 字形を本物の文字に置き換える (対応表 + 利用者の追加) |
| `arib_drcs_map.h` | その対応表 (**自動生成**。regen-drcs-map.sh) |
| `drcs_store.cpp` | 置き換えられない字形を貯めて、次の起動で登録する |
| `drcs_font.cpp` | フォントを本体に登録する (初期化時のみ) |

配置は `bridge.cpp` の `PlaceCaptions()`。**字幕文ではなく 1 行**につき
「テキスト」オブジェクトを 1 つ作り、`[Caption] Layer` から下の
レイヤーへ、映像の先頭からの秒数に合わせて並べる
(放送は行ごとに座標を持っている為。「行ごとに 1 つのオブジェクトに
する」を参照)。
...
放送のルビは**小型 (SSZ) で本文の 1 行上に、まとまりごとに位置を
打って**書かれる。そのままだと本文の頭にふりがなだけが並ぶ
(`<s><s*0.5>あいぞううずま<s>♬ 愛憎愛憎渦巻いて`)。
...
**1 文字ごとの X で対応付ける。**その為に `PushText()` の
まとめ書きをやめ、`AribItem::D` に「書き始めの X」を持たせた
(まとめると 1 文字ごとの位置が判らなくなる)。
変換後の文字列は繋げるだけなので変わらない。

`AribItemsToAviUtl2()` は小型の文字をルビとして溜め、行を閉じる時に
`</>` … `<!>ふりがな</>` で括る。**後ろから入れる事** (前から入れると
後ろの位置がずれる)。

```
♬ </>愛憎<!>あいぞう</>愛憎</>渦巻<!>うずま</>いて
```
...
の順で来る。受け取った時点の行に付けると**2 行目のルビが 1 行目に
乗る** (実機で発生)。`Pending` に預けておき、**行の最初の字が来た所で
引き取る**。

**掛ける字は「その字の半分以上に重なっているか」で決める。**
ちょうど半分も含める。外すと 1 文字のルビがどの字にも掛からず
消えてしまう (「然(さ)らば」のルビ「さ」は 20 ドットで、
字幅 40 のちょうど半分)。
...
| | ルビの幅 | 本文 | 次の字の重なり |
| --- | --- | --- | --- |
| 渦巻(うずま) | 60 | 渦 40 + 巻 40 | 20 → 含めたい |
| 狂騒(きょうそう) | 100 | 狂 40 + 騒 40 | 20 → 含めたくない |

どちらも左揃えで重なりは 20 ドット、**放送のバイト列は同じ形**
(実測で ACPS の値まで確かめた)。ルビが本文より広い時だけ 1 字
余分に括られるが、ルビが丸ごと消えるよりは良いと判断した。
`[Caption] Ruby=0` で従来どおり本文に混ぜる形に戻せる。

#### 表示の長さは「消す」データユニットから決める

放送は**本文の無いデータユニット (画面消去 CS だけ)** で
「ここで消す」と送って来る。以前は本文の無いユニットを捨てていた為、
**次の字幕が来るまで出しっぱなし**になっていた。

`TSMemoryCaption::EndSeconds` を足し、`CTSCaptionSource` が
次の規則で埋める。

> **画面消去 (CS) を含むか、本文のあるデータユニットが来たら、
> それまでに出ていた字幕はそこで消える。**

- **本文が空でも CS が無ければ消さない。**属性を変えただけの
  データユニットで消えてしまう為
- 判らなかった時 (取り込んだ範囲の最後の 1 件など) は、
  従来どおり `PlaceCaptions()` が「次の字幕まで」で代用する。
  両方ある時は**早い方**を採る

実測 (`test_caption` が字幕を持つ 4 番組で出す):

```
                          消える時刻   放送の表示  次の字幕まで  差
Record_20260827-013024.ts  6 件 (1 件不明)  3.9 秒     3.9 秒    0.0
Record_20260827-013212.ts  3 件 (1 件不明)  4.3 秒     4.3 秒    0.0
Record_20260827-110354.ts  1 件 (1 件不明)  3.0 秒     5.4 秒    2.4
Record_20260827-140438.ts  7 件 (1 件不明)  3.0 秒     3.0 秒    0.0
```

**ほぼ全ての字幕で消える時刻が拾えている** (不明なのは各ファイルの
最後の 1 件だけ)。多くの局は消去を次の字幕と同じ時刻に送るので
差は出ないが、間が空く番組では 2.4 秒長く出たままになっていた。

`test_caption` は同じ規則をなぞって
`most captions have an explicit erase time` を検査する。
CS の判定が壊れるとここが落ちる。

#### レイヤーは「字幕が取れるかを調べる前」に空ける

**前のチャンネルの字幕が残っていた** (実機で発生)。
映像は切り替わるのに字幕だけ古いまま、という形で出る。

映像側と字幕側で空ける順序が違ったのが原因。

| | 空けるタイミング |
| --- | --- |
| 映像 (`ReplaceLayer`) | 新しい映像が取れるかに関わらず**先に**空ける |
| 字幕 (直す前) | **字幕が取れた後**にしか空けない |

字幕側は手前に早期 return が並んでおり、そこを通ると古い字幕が
残ったままになる。特に `CTSCaptionSource::Open()` の
「字幕のストリームが見つかりません」に落ちやすい。
**チャンネルを変えると TVTest 側が `PurgeBuffer()` でバッファを
空にする**ので、直後は数秒分しか溜まっておらず、
字幕は 3〜4 秒間隔なので PMT と字幕文が揃っていない事が普通にある。

もう 1 つ、空ける本数が「今回必要な本数 (`MaxLines`)」だった。
前回 3 行・今回 1 行なら**レイヤー 2〜3 に前回の字幕が残る**。

`ClearCaptionLayers()` に切り出し、**`Source.Open()` より前**に呼ぶ。

- 消すのは **`Layer` から下へ続く塊だけ**。空のレイヤーに当たったら
  そこで止める。字幕は `Layer` から隙間なく置くので、空きから先は
  字幕の持ち物ではない (利用者が置いた物を消さない為)
- 上限は `MAX_CAPTION_LAYERS` (8)
- **映像のレイヤーに当たったら止める。**ここで消すと映像ごと消える
- ロックの扱いは今までどおり (前回掛けた物は外す / 手で掛けた物は
  外さず諦める)

今回の方が行数が多い場合、塊の先の空のレイヤーはこの関数を抜けている。
そちらのロックは `MaxLines` が判ってから改めて見る。
...
#### AviUtl2 の制御文字を仕様と突き合わせる (出力側)

`<tw>` の誤りをきっかけに、出している制御文字を `aviutl2.txt` と
入力補助の既定 `default.word` に全て突き合わせた。**4 件が誤りだった。**

| 制御文字 | 仕様 | 判定 |
| --- | --- | --- |
| `<#rrggbb>` `<#rrggbb,rrggbb>` | 文字色, 影縁色 | ✔ |
| `<s数値>` | 絶対サイズ | ✔ |
| `<s*n>` | 相対サイズ | △ 基準が不明。**必ず `<s>` で戻してから掛ける** |
| `<@名前>` `<@>` | フォント | ✔ |
| `<$名前>` | プリセット | ✔ |
| `<tw n>` | **横スケール (倍率)** | ✘ `<tw50>` = 50 倍だった |
| SZX 0x41 | **縦だけ 2 倍** | ✘ 縦横 2 倍にしていた |
| SZX 0x44 / 0x45 | 横だけ 2 倍 / 縦横 2 倍 | ✘ 標準に戻していた |
| 中型 → 小型 の遷移 | | ✘ `<tw0.5>` が残っていた |
| C0 の空白 (0x20) | 1 文字分の枠 | ✘ 半角の空白にしていた |
| `<` の打ち消し | **仕様に記述が無い** | ✘ `<<` と当て推量していた |

**`<tw>` は百分率ではなく倍率。**`aviutl2.txt` の例が `<tw0.8>`、
`default.word` の既定も `<tw0.8>`。中型 (MSZ) を `<tw50>` と書いていた為、
**50 倍に引き伸ばされた文字が横一線に潰れて画面を横切っていた**
(実機で「楕円のような線」として現れた)。`obj.w` も巨大になるので
スクリプト側の位置合わせも同時に壊れる。

**大きさは縦横を別に持つ。**SZX には「縦だけ 2 倍」「横だけ 2 倍」が
あり、1 つの値では表せない。`AribItem::Size` は
A = 横スケール / B = 縦スケール (どちらも 10 倍した整数) にした。
...
| SZX 0x45 | 2.0 | 2.0 | `<s><s*2>` |

`<s>` は縦横の両方に効くので、**縦を `<s>` で決めてから横との比を
`<tw>` で掛ける**。この形なら遷移でどちらかが残る事も無い。

**`<` の打ち消し方は仕様に無い。**`aviutl2.txt` にも `default.word` にも
記述が無いので、`<<` のような当て推量は使わず**全角の `＜` に
置き換える**。字幕の本文はもともと全角で、英数集合も全角で出している
ので ASCII の `<` はまず来ない。
読む側でも 1 件直した。**`90 20 4n` は色ではなく
色配列 (CLUT) の選択。**これを前景色として扱うと**字幕がほぼ全て
同じ色に染まる** (実測では全ての字幕文が `<#0000ff>` = 青字に
なっていた)。放送 25 番組の字幕文 36 件を調べたところ、この形は
...
こちらでは ORN を `AribItemType::Ornament` として取り出しているが、
**縁取りそのものは放送の指定に頼らずスクリプト側で付ける**
(TVCaptionMod2 と同じ考え方)。

##### 縁取りをどこで付けるか

AviUtl2 で縁取りを出す道は 2 つある。

**(a) テキストの制御文字。**`aviutl2.txt` に記述がある。

```
◇フォント、文字装飾の変更
        <@[フォント名][,文字装飾(0～6/B/I/S)]>
        文字装飾 : 0=標準文字 / 1=影付き文字 / 2=影付き文字(薄)
                 / 3=縁取り文字 / 4=縁取り文字(細) / 5=縁取り文字(太)
                 / 6=縁取り文字(角)
◇サイズの変更
        <s[文字サイズ][,フォント名][,スタイル][,縁取りサイズ]>
```

縁の色は `<#文字色,影縁色>` の 2 つ目で、これは既に出している。
**採らなかった。**文字装飾はプリセットと制御文字の持ち物なので、
字幕の側から書くと利用者の書体設定と喧嘩する。書体名を省いた
`<@,3>` が通るかも仕様に書かれておらず (`default.word` の例も
`<@メイリオ>` だけ)、外字の後に出している `<@>` が装飾まで
戻してしまう問題もある。

**(b) スクリプトから本体のフィルタ効果を呼ぶ。**採ったのはこちら。
`lua.txt`:
...
`TSMemory字幕背景.anm2` が背景を敷く前に `obj.effect("縁取り", ...)`
を呼ぶ。プリセットに手を入れずスクリプトだけで完結し、
トラックバー「縁取り」と色設定「縁色」で調節できる。
`[Caption] BackOutline` (既定 2) から入れ直す。
**背景を切っていても縁取りだけ付けられる**ように、
`BackOpacity > 0 || BackOutline > 0` でスクリプトを付ける。

**フィルタ効果の項目名は本体が持っている物で、静的には確かめる術が
無い。**`effect.conf` の `[OldScript.縁取り] color=縁色` と
`aviutl2.exe` の文字列 (「縁取り」「縁色」「パターン画像」) から
`"サイズ"` / `"縁色"` と組み立て、`pcall` で包んで
**失敗しても字幕そのものは出る**ようにした上で実機に投げた。

実測 (`[Caption] BackDebug=1`、AviUtl2 のログ):

```
TSMemory字幕背景: 縁取り 成功 size=2 216x88 -> 220x92 ox 0 -> -2 oy 0 -> -2
TSMemory字幕背景: text=220x92 padx=8 pady=4 alpha=50 bgcol=000000
TSMemory字幕背景: 合成後 obj=236x100 ox=-10 oy=-6
```

- **項目名は `"サイズ"` / `"縁色"` で合っていた** (「成功」)
- `size=2` で `216x88 -> 220x92`。**四方にちょうど 2 ドットずつ広がる**
- `ox 0 -> -2`。**本体の縁取りフィルタは `obj.ox` を触らない**ので、
  スクリプト側の補正が働いた。基準点が左上のままだと文字が
  右下へ 2 ドット動いてしまうので、この補正が要る

`pcall` と「ox が動いていない時だけ直す」判定は、
**確認が取れた後も残してある。**本体側の版が変われば
どちらも変わり得る所で、外すと黙って壊れる。

##### 縁の太さの既定 (2 ドット) の根拠

**TVCaptionMod2 の縁は文字の大きさに対する相対**で、こちらは
絶対のドット数という違いがある。`src/TVCaption2.cpp`:

```cpp
osd.SetStroke(strokeWidth < 0 ? min(strokeWidth * (nomalHeight + charHeight) / 2, -1)
                              : strokeWidth * 72, ...);
```

`StrokeWidth` の既定 -2 は負なので相対側に入り、`CPseudoOSD` では
...
と 1/72 単位で膨張の回数に直される。標準の 36 ドット文字を
1080p (字幕平面 540 の 2 倍) で出す場合はおよそ 2 回 = **2 ドット**で、
既定値をここに合わせている。

**文字の大きさにも追従させている。**スクリプトの
「縁を文字に合わせる」(既定で有効) が `obj.h` で割り直す。
基準は「1080p で放送どおりの大きさ (`<s72>`) の 1 行」= `obj.h` 88
(実機のログに出ていた `216x88` の値)。
...
1080p で放送どおりの大きさなら係数は 1.0 なので、既定値そのままになる。

#### ARIB の制御符号を全件洗う (入力側)

`SDF` の抜けをきっかけに、**ARIB STD-B24 の制御符号を全て**
libaribcaption の一覧 (`b24_controlsets.hpp` / `b24_codesets.cpp`) と
突き合わせ、**実データでの出現数も測った**。

計測は使い捨ての計測器 (復号器に数える所だけ足した物) で、
放送 33 番組 (呪術廻戦 12 / LV999 4 / あかね噺 3 / 乙女怪獣 2 /
LIAR GAME / SPY×FAMILY / Record_* 5 等)。単体検査ぶんは差し引き。

**実際に来た符号** (これらは全て対応済み):

```
[C0]  NUL 14 / CS 3628 / LS0 470 / ESC 2840 / APS 806 / SS3 470 / SP 1668
[C1]  前景色 846 / SSZ 2046 / MSZ 4852 / NSZ 5412 / COL 5712
      FLC 2 / CSI 24812 / TIME 410
[CSI] SWF 2564 / SDF 3012 / SSM 3012 / SHS 3012 / SVS 3012 / SDP 3012
      ACPS 4898 / ORN 2290
[集合] ひらがな 470 / カタカナ 470 / 漢字 470
[外字] 0x41 (外字 1 番) 274 / マクロ 470
```

**一度も来なかった符号**は次のとおり。対応していない物もあるが、
実害は出ていない。

| 符号 | 状態 |
| --- | --- |
| APB / APF / APU / PAPF | 送りに反映するようにした (下記) |
| APR / APD (改行) | 位置指定として出すようにした (下記) |
| RPC (文字の繰り返し) | **対応した** (下記) |
| PLD / PLU (半行下・半行上) | **対応した** (下記) |
| MDF (太字・斜体) | **対応した** (下記) |
| HLC (囲み) / STL・SPL (下線) | 復号するが**出せない** (下記) |
| MACRO (C1) | **0x4F まで読み飛ばすようにした** (下記) |
| CDC / POL / WMM | 引数を読み飛ばすだけ (見た目のみ) |
| GSM / CCC / GAA / SRC / TCC | 未実装 (下記) |
| CFS / XCS / SCR / PRA / ACS / UED / RCS / SCS | 未実装 (参照側も無視) |
| プロポーショナル集合 (0x36-0x38) | 対応した (下記) |
| ラテン拡張・特殊 / モザイク A-D | 未実装 (日本の放送では使わない) |

##### 潜んでいた誤り 2 件

**外字の指定を本文の集合として読んでいた。**
`ESC ... 0x20 F` の `F` は外字の番号で、本文の集合とは意味が違う。
同じ表で引いていた為、

| F | 正しくは | 誤って |
| --- | --- | --- |
| 0x42 | 外字 2 番 (1 バイト) | 漢字集合 (2 バイト) |
| 0x49 | 外字 9 番 | JIS X0201 カタカナ |
| 0x4A | 外字 10 番 | 英数 |

漢字集合と読むと以降が 2 バイトで解釈され、**本文が丸ごと化ける**。
`FromFinal()` に `fDrcs` を足して表を分けた。
実測では外字 1 番 (0x41) しか来ないので表には出ていなかった。

**プロポーショナル集合を捨てていた。**
`0x36` / `0x37` / `0x38` が `CharSet::Other` に落ち、
**その集合の文字が丸ごと消えていた**。字形の表は普通の集合と同じ
(送り幅だけが字ごとに変わる) ので、英数・ひらがな・カタカナに
割り当てた。

##### 出たら壊れる物を先に潰す (RPC / PLD / PLU)

出現 0 件だが、**来たら SDF と同じく本文か位置が壊れる**物。

**RPC (文字の繰り返し)。**回数は `P1 - 0x40` で、**0 なら行末まで**。
caption.dll (TVCaptionMod2 が使っている物) の解釈に合わせた。
...
無視すると 1 文字しか出ず、その行の以降の位置も繰り返した分だけ
左にずれる。文字を書く所を `PutText()` / `PutDrcs()` にまとめ、
折り返しと繰り返しをここで見るようにした。

**PLD / PLU (半行下・半行上)。**上付き・下付きや、局によっては
ルビに使われる。無視すると本文と同じ行に混ざる。
**半行なので行の区切りにはならない** (変換側の閾値は行送りの 3/4)。

##### 装飾 (MDF / HLC / STL・SPL)

**太字と斜体 (MDF) は出せる。**`aviutl2.txt` の
「フォントスタイルの変更」で `<@+B>` / `<@-B>` / `<@+I>` / `<@-I>`。

**下線 (STL / SPL) と囲み (HLC) は出せない。**
`aviutl2.txt` の文字スタイルは **B (太字) / I (斜体) /
S (取り消し線) の 3 つだけ**で、下線も囲みも制御文字が無い
(「下線」「アンダーライン」の語が `aviutl2.txt` に 1 つも無い)。
`AribItemType::Decoration` のビットとして復号はするが、
出力には使わない。

参考までに caption.dll は**下線を太字で代用**している
(`//STL アンダーライン モザイクの開始` → `m_bBold = TRUE`)。
紛らわしいので真似ていない。

やるとすれば「TSMemory字幕背景」スクリプトに線を引かせる形になるが、
スクリプトは行単位なので**行の一部だけの下線は表せない**。
放送でも一度も来ていないので見送っている。

##### MACRO の定義は 0x4F まで抱えている

 (C1 0x95) を「引数 1 つ」として食べていた。マクロの**定義**は
 までの並びを抱えているので、**定義の中身が本文に混ざる**。
 と同じく、来たら本文が壊れる類。

参照実装はどちらも特別扱いしている。

- caption.dll … 
- libaribcaption … 「ARIB TR-B14 では使わない」として
   (その字幕文ごと捨てる)

caption.dll に合わせて  まで読み飛ばす。
**見つからない時は 1 バイトだけ食べて先へ進む**
(壊れたデータで残り全部を飲み込まない為)。実測では 0 件。

##### GSM を見送る理由

`GSM` (文字変形) は送り幅に効くので、来たら位置がずれる。
しかし**引数の意味を確かめられなかった**。

- libaribcaption … `case CSI::GSM: break;` (無視)
- caption.dll … `//GSM` `//未サポート`

`CCC` (合成) も同じで、**caption.dll も `//CCC` `//未サポート`**、
libaribcaption も `case CSI::CCC: break;` だった。同じ理由で見送る。

**どちらの参照実装も実装していない。**推測で書くと、来た時に
今より悪くなり得るので入れていない。引数のバイト数は元から
正しく読み飛ばせているので、**以降が化ける事は無い**。

##### ペンを動かす符号

`APB` / `APF` / `APU` / `PAPF` / `APD` / `APR` を `MovePen()` に
まとめ、桁・行の単位で `PenX` / `PenY` を動かして
`AribItemType::Position` を出すようにした。
`PAPF` は引数を捨てるだけだった為、来ていたらその分だけ以降の位置が
左にずれていた。座標が判っていない間は従来どおり
`AribItemType::LineBreak` で伝える。

#### 行送りは「文字を書く時」の大きさで決まる

2 行の字幕が**半行ぶん重なっていた** (実機で発生)。

**放送は位置を打ってから大きさを指定して来る。**実測の生バイト
(『呪術廻戦』第 3 期 #50):

```
CSI "250;389" SP 'a'   ルビの位置
88                     SSZ (小型)     ← 位置の後に大きさ
ゆ う た
CSI "170;449" SP 'a'   1 行目の位置 (この時まだ小型のまま)
8A                     NSZ (標準)
乙 骨 憂 太
CSI "170;509" SP 'a'   2 行目の位置
九 十 九 由 基 …
```

行送りを**位置指定の時点**の大きさで決めていた為、1 行目が小型の
送り 30 で計算され、上端が 449+1-30 = 420 になっていた
(正しくは 449+1-60 = 390)。2 行目は 450 なので、**間隔が
半行の 30 しか無く重なる**。

大きさが変わった時に、**まだ文字を書いていない位置指定を入れ直す**
(`RetunePosition()`)。文字を書いた後の位置指定には触らない。

```
直す前  : 上420 と 上450  (間隔 30 = 半行)
直した後: 上390 と 上450  (間隔 60 = 1 行)
```
...
**1 行に収まらない字幕は、放送側が折り返し前提で送って来る。**
`SDF` (`CSI w;h SP 'V'`) が表示領域の大きさで、`SDP` の左上と
合わせて「どこで折り返すか」が決まる。読んでいなかった為、
**2 行の字幕が 1 行に繋がっていた** (実機で発生)。

実測 (『呪術廻戦』第 3 期 #50):

```
SDP 170;30 / SDF 620;480   -> 右端は 170 + 620 = 790

直す前 : (左170 右1230 上390)   ← 平面は 960 幅なのに 1230 まで伸びた
         聞きに来ました｡ 知<tw0.5>っ<tw>ていることを話してもらえませんか？

直した後: (左170 右790 上390) と (左170 右610 上450) の 2 行
         聞きに来ました｡ 知<tw0.5>っ<tw>ていることを
         話してもらえませんか？
```

文字を書く直前に `Wrap()` を呼び、`PenX + PitchX` が右端を超えるなら
`PenX` を左端へ、`PenY` を 1 行下へ動かして
`AribItemType::Position` を出す。**ちょうど収まる字は折り返さない。**

その為に `State` に `PenY` を足した (折り返した先の Y を作らないと、
変換側は行が変わった事に気付けない)。`APS` / `ACPS` でも `PenY` を
覚えるようにしてある。

#### 改行は座標から起こす

**放送の字幕は改行を送って来ない。** 1 行ごとに座標を打つ形なので、
位置指定を読み飛ばすと**全ての行が繋がって 1 行になる**。

C0 制御符号の出現数を数えた (字幕を持つ 4 番組。単体テストぶんは
差し引いてある):

```
                            013024  013212  110354  140438
  0x1C APS  (行桁指定)          12      10       4      16
  0x0D APR  (改行)               0       0       0       0
  0x0A APD  (1 行下)             0       0       0       0
  0x0B APU  (1 行上)             0       0       0       0
  0x09 APF  (1 つ進む)           0       0       0       0
  0x08 APB  (1 つ戻る)           0       0       0       0
  0x16 PAPF (n つ進む)           0       0       0       0
```

**位置指定は APS だけ**で、改行符号も相対移動も一切来ない。
APS の数は変換後の行数とちょうど一致する
(`AribItemType::Position` の数と同じ)。

`CSI x;y SP 'a'` (ACPS) を使う局もある為そちらも読むが、
**この 4 番組では一度も現れなかった**。どちらも
`AribItemType::Position` に揃えてから扱う。`ParseCsi()` で
...
> `%ProgramData%\aviutl2\Default\<スクリプト名>.effect` があると
...
割らないと離れた場所の文字が 1 つのオブジェクトに繋がり、間の空白が
消えて背景の箱も両方をまたいでしまう。
実測では放送 14 番組中 4 番組で発生していた
(「アニメ 響け!ユーフォニアム」で 103 行中 2 行が横並びで割れた)。
**書き終えたすぐ隣に置き直しただけの時は割らない事**。
放送は 1 行の中でも位置を打ち直すので、それを割ると逆に壊れる。
...
**この分け方は放送のデータの形に沿っている。**上の C0 の計数どおり、
行の切れ目は必ず APS (絶対座標) で、APS の数と行の数は一致する。
改行符号として送られている物を分解しているのではなく、
**元から別々に置かれている物をそのまま別々に置いている**。

1 つのオブジェクトに `\n` で詰めると次が表せない。

| 放送が持っている物 | `\n` でまとめた場合 |
| --- | --- |
| 行ごとの X 座標 | 先頭行の X に揃ってしまう |
| 行ごとの文字色 (話者ごとに違う) | 制御文字で書けるので表せる |
| 行ごとの背景の箱 | 全体を囲む 1 つになる |
| 同じ高さで横に並ぶ 2 か所 | **表せない** (改行ではない為) |

**ただし ARIB の本当の単位は「行」ではなく「文字のマス」。**
1 文字ごとに升目 (36 ドット + 字間 4 = 送り 40) の上に置かれていて、
「行」はこちらが座標から起こした括りにすぎない。ここは近似が 2 つ残る。

- **行の中の文字の位置は書体任せ。**固定するのは行の左上だけで、
  以降は AviUtl2 の字送りに従う。書体の送り幅が升目と違えば
  行の中で少しずつずれる (TVCaptionMod2 は `lfWidth` に
  文字幅を直接与えて升目に合わせている)。
  升目どおりにするなら `<p x,y>` で 1 文字ずつ置く事になるが、
  **テキストとして編集できなくなる**ので採っていない。
- **行の括り方は座標からの推定。**「行送りの 3/4 以上下がったら改行」
  「小型 (SSZ) はルビなので行にしない」「同じ高さで送り幅より
  離れたら別の行」という規則で分けている。データに
  「ここが 1 行」という印は無い。
...
**基準点は行の左上。**テキストオブジェクトの文字揃え「左寄せ[上]」が
そのままの意味だった (実機で TVCaptionMod2 と並べて確認済み)。
放送の字幕も左揃えなので、`X` に行の左端、`Y` に行の上端を入れればよい。

> **目測で基準点を決めようとして遠回りした。**
> プレビュー画面から左端の位置を目分量で読み、「X は中央を指す」と
> 誤って結論して、描画後の幅の半分だけずらす仕掛けまで入れてしまった。
> **数十ピクセルの差を screenshot の目視で判断してはいけない。**
> 設定で切り替えられるようにして実機で見比べたら 1 回で決着した。

行の右端も出しているが、これは背景の箱と、横並びの判定に使う。
右端は**ペンの進み** (文字ごとに `PitchX()` だけ進める) で決まるので、
...
`TSMemoryRegisterFontCollection()` は初期化中だけ許すようにした
(`TSMemoryFontCanRegister()`)。

**なぜ今まで落ちなかったか。**符号の形が違っていて字形が 1 つも
揃わず、組み立てたフォントが空のままだった為に登録を呼んでいなかった。
符号を直した途端に初めてこの経路が動き、落ちるようになった。

##### まず対応表で本物の文字にする (採った方法・その 1)

**放送の外字の多くは、Unicode に実在する文字をビットマップで
送っているだけ。**libaribcaption の `kDRCSReplacementMap` が
「字形の md5 → Unicode」の対応表を持っている。

```
290 通りの md5 -> 99 文字
  ⁈⁉♬➡『』〜㊙｟｠ … 記号
  德 髙 﨑 塚 𠮷 神 剝 塡 … 人名の異体字
```

置き換えてしまえば**フォントが要らない**ので、その回の取り込みから
出る。普通の文字なので編集も検索もでき、縁取りも色も本文と同じに効く。
`caption/drcs_replace.*` と、生成物の `caption/arib_drcs_map.h`
(`tools/regen-drcs-map.sh` で作り直す) が担当する。

**md5 は生のビットマップだけを対象に取る。**幅・高さ・深さは含めない。
libaribcaption と同じ取り方にしないと 1 件も引けない。我々の DRCS の
読み取りは元々一致していた (どちらも 36x36 深さ 4 で 324 バイト)。
md5 は Windows の BCrypt で取るので、外から持って来る物は要らない。

引けなかった字形は md5 をログに出す。利用者が ini と同じ場所の
`TSMemoryDrcsMap.txt` に `md5 = 文字` と書けば、**再起動なしで**次の
取り込みから出る。利用者の分を組み込みの表より先に見るので、
合わない字形の上書きにもなる。

##### 残りは字形を貯めて次の起動で登録する (採った方法・その 2)

**駄目なのは「取り込みの時に呼ぶ事」だけ**で、初期化の中でなら
`register_font_collection()` は通る (上の表)。字形をファイルに貯めて
おけば、次の起動の `RegisterPlugin` でまとめて渡せる。

`caption/drcs_store.*` が担当する。

| する事 | いつ | どこ |
| --- | --- | --- |
| 利用者の対応表 `TSMemoryDrcsMap.txt` を読む | `RegisterPlugin` | `drcs_replace` |
| `TSMemoryDRCS.dat` を読んで枠の並びを復元 | `RegisterPlugin` | `drcs_store` |
| そこから TTF を組み立てて登録 | `RegisterPlugin` | `drcs_store` |
| 対応表で引く。駄目なら枠に結び付ける | 取り込み | `ts_caption` |
| `.dat` を書き直す | 取り込みの最後 | `drcs_store` |

**ファイルはプラグインの ini と同じ場所にしか置かない。**
`ProgramData\aviutl2\Font` に TTF を置く方法でも動く (本体が起動時に
読む。自前の TTF を置いて起動し、書体一覧に出る事を実機で確認済み) が、
`register_font_collection()` はメモリのまま受け取れるので**ファイルを
作らずに済む**。次の起動から、という点はどちらも変わらないので、
システム側のフォルダを触らない方を採った。

**鍵は ARIB の符号ではなく字形そのもの** (深さ・幅・高さ・
ビットマップを繋げた文字列)。符号 (`0x21` から順) の意味は番組ごとに
変わるので、符号を鍵にすると別の番組で違う字が出る。中身で引けば
取り違えようがなく、同じ字形は必ず同じ枠に落ちる。

枠 `n` は `U+E000 + n` に対応する。**本体が起動時に読んだ数
(`TSMemoryDrcsStoreLoadedCount()`) より後ろの枠は、今のフォントには
まだ入っていない。**その字は今回だけ代替文字にし、次の起動から出る。

上限は 512 字形 (`STORE_MAX`)。実測の字形は 36x36 の 4 階調
(2 ビット/画素) で 1 字形 324 バイトなので、上限まで貯めても 166KB。

**上限に達した時は「次の起動から出る」ではない。**枠が無いので
再起動しても出ない。`m_StoreFullGlyphs` として別に数え、
「`TSMemoryDRCS.dat` を消すと貯め直します」と案内を分けている
(混ぜると嘘になる)。対応表が効くようになった今、ここに達するのは
まず無い。

**付け替えは 1 回の走査で行う。**変換の時点では枠が判らないので、
本文には「参照の順番」で仮の私用領域の文字が入っている。順に置換すると
付け替えた先を次の回で拾ってしまう (0 番 → 2 番、2 番 → 0 番 のような
入れ替えで壊れる)。
...
```
外字の内訳 : … 蓄積 2 字形 / キャッシュ 2 件 : 0021=無し 0022=無し
外字 2 字形を字幕の蓄積から拾いました
外字 2 個は字形が届いていません
```

字形は 2 つ受け取れているのに、本文の 0x21 / 0x22 と結び付いていない。

**定義側の符号は 2 バイトの `character_code`** で、
1 バイト外字 (データユニットのパラメータ 0x30) の場合は
...
という作りになっている。そのまま 2 バイトを鍵にしていた為、
**本文の `0x21` と定義の `0x0121` が一致しなかった。**

libaribcaption も同じ分解をしている (`decoder_impl.cpp`):

```cpp
if (byte_count == 1) {
    uint8_t index = ((character_code & 0x0F00) >> 8) + 0x40;  // 集合
    uint16_t ch   = (character_code & 0x00FF) & 0x7F;         // 符号
```

2 バイト外字 (0x31) は `0x7F7F` でマスクする
(`0xEC00`〜`0xF8FF` はそのまま)。

**実測の放送は 1 バイト外字しか使わない**ので、外字は
**一度も出た事が無かった**。直した後は定義と本文の符号が揃う。

```
定義された外字の符号 : 0x0021(19件) 0x0022(1件)
外字の符号           : 0x0021(20件) 0x0022(11件)
```

`test_caption` が両方を出すので、ずれたら気付ける。

#### 外字の符号に決まった意味は無い (二重かっこが化ける件)

実機で二重かっこが代替文字になった。ログには
「外字 2 個は字形が届いていません」と出ていた。

**この 2 つは外字 (DRCS) として送られている。**後に md5 で引いて
判ったが、正体は `｟` `｠` (U+FF5F / U+FF60) だった。

符号を数えた (『呪術廻戦』第 3 期 12 本):

```
0x0021(26件) 0x0022(12件) 0x0023(4件) 0x0024(2件)
0x0021(30件) 0x0022(14件)
0x0022(1件)
```

**`0x21` から順に割り当てているだけ**で、番組ごとに中身が違う。
`0x21` が `｟` とは限らない。**符号を鍵にした対応表は作れない。**

> **字形を鍵にすれば作れる。**当時はここで諦めていたが、
> ビットマップの md5 を鍵にすれば番組が変わっても当たる。
> 実際 libaribcaption がその表を持っていた
> (「まず対応表で本物の文字にする」を参照)。**鍵の取り方の問題で
> あって、対応表そのものが無理だったわけではない。**

字形そのものは流れて来ている (同じ 12 本で、参照 44 件に対して
定義のデータユニットが 26〜31 件)。**問題は窓に入るかどうか**で、
`MemorySize` を大きくすれば拾える (下記)。

##### 字幕だけを TVTest 側で長く溜める

**見えている範囲が違う。**

| | 字幕ストリームの見え方 |
| --- | --- |
| TVTest 側 | **常時**全パケットが流れて来る |
| AviUtl2 側 | 取り込んだ瞬間のリングバッファ (8〜14 秒) だけ |

字形の再送間隔 (中央値 14.2 秒) に対して AviUtl2 側の窓が短いので、
**AviUtl2 側でいくら覚えても取り込みの瞬間に入っていた分しか
溜まらない**。TVTest 側で溜めるのが筋。

字幕は取り込む量の **0.01〜0.02%** (実測) しか無いので、
別のリングを長く持っても安い。

- `CTSMemory` に `m_CaptionRing` (普通のメモリ) を持つ。
  大きさは `[Settings] CaptionMemorySize` (MB、既定 1)。
  1MB で数十分ぶん入る
- `InputMedia()` で**字幕 PID のパケットだけ**を追加で入れる。
  PID は `TVTest::ServiceInfo::SubtitlePID` から取れるので、
  PMT を自前で解析しなくてよい
- 取り込みの時に `<名前>.caption` という共有メモリへ写す
  (`BufferInfo` の作りは本編と同じ)
- チャンネルを変えたら `PurgeBuffer()` で一緒に捨てる
  (符号の意味が変わる為)

**AviUtl2 側は本文を取らない。**`<名前>.caption` は古いパケットも
含むので、字幕文まで拾うと重複したり時刻が合わなくなる。
`fGlyphsOnly` を立てて**字形の定義 (0x30/0x31) だけ**を拾い、
その後に本編を流して新しい字形で上書きさせる。

`.caption` が無い時 (古い `TSMemory.tvtp` と組み合わせた時) は
黙って何もしない。拾えた数はログに出す。

**取り込みをまたいで字形を覚えておく事でも埋める。**
`ts_caption.cpp` に `g_GlyphCache` (AviUtl2 の起動中だけ持つ) を置き、
受け取った字形を全て入れる。次の取り込みで窓に入らなかった符号は
ここから引く。`[Caption] DrcsCache` (既定 1) で切れる。

**符号の意味は番組ごとに変わるので、そのままでは取り違える。**
2 つで絞っている。

- **鍵にサービス ID と字幕 PID を含める**
  (`(サービス ID << 32) | (PID << 16) | 符号`)。
  **PID だけでは足りない。**別のチャンネルでも同じ番号 (0x0110 等) を
  使っている事が多く、チャンネルを変えた後に前のチャンネルの字形を
  引いてしまう。サービス ID は PMT の program_number から取る
- **30 分より古い物は使わない** (`GLYPH_CACHE_LIFE_MS`)

それでも同じチャンネルで番組をまたぐと取り違え得るので、
**使った時はログに出す** (「外字 N 個は前回までに受け取った字形を
使いました」)。おかしな字が出たら `DrcsCache=0` にしてもらう。
...
`CTSCaptionSource` を作るのはその後。復号も外字の処理も走らない。
...
component_tag 0x38〜0x3F は取っていない (`ts_caption.cpp` で除外)。
緊急情報等で性格が違う為、取るとしても別の設定にする。

**出力に使っていない制御符号**

復号はしているが、AviUtl2 側に表す手段が無いか、静止画では
意味が無いもの。**位置も文字数も変わらない**ので実害は無い。
出現数は 33 番組の実測。

| 符号 | 理由 | 出現 |
| --- | --- | --- |
| ORN (縁取りの色) | 縁はスクリプトで付けている。色は常に黒だった | 8 本中 2 本 |
| HLC (囲み) / STL・SPL (下線) | **AviUtl2 の制御文字に無い** | 0 件 |
| FLC (点滅) | 静止画では意味が無い | 2 件 |
| POL / WMM / CDC | 見た目だけの指定 | 0 件 |

**GSM (文字変形) と CCC (合成)**

来たら送り幅や文字数がずれるが、**参照実装が 2 つとも未サポート**で
引数の意味を確かめられない。

| | GSM | CCC |
| --- | --- | --- |
| libaribcaption | `case CSI::GSM: break;` | `case CSI::CCC: break;` |
| caption.dll | `//未サポート` | `//未サポート` |

推測で書くと来た時に今より悪くなり得るので入れていない。
**引数のバイト数は正しく読み飛ばせている**ので、以降が化ける事は無い。
やるなら実際に送って来る放送を見つけて挙動を測ってから。
...
- **字幕は既定で無効です。** 有効にするには**両方**の設定が要りますが、
  ...
  済みます。詳細は上の「字幕対応」。
  **文字スーパー**と、AviUtl2 に表す手段の無い**下線・囲み**は出ません。
