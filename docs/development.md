...
> **基準点はまだ確定していない。**
> 目測に頼ったので判断が揺れている。`[Caption] AlignLeft` /
> `AlignTop` で切り替えられるようにしてあるので、実機で見比べて
> 決める事。決まったら設定を落として既定に固定する。
>
> | | 1 (既定) | 0 |
> | --- | --- | --- |
> | `AlignLeft` | X は行の中央を指す。幅の半分ずらす | X は行の左端を指す。何もしない |
> | `AlignTop` | Y は行の中央を指す。高さの半分ずらす | Y は行の上端を指す。何もしない |

**しかし中央で合わせても揃わない。**
放送の字幕は左揃えで、放送側の幅と AviUtl2 の描画幅は書体が違うので
一致しない。中央を合わせると差の半分だけ左端がずれ、行ごとに
文字数が違う為に**縦のラインがガタガタになる** (実測)。

描画後の幅は AviUtl2 の中でしか判らないので、
**`X` には行の左端を入れ、`TSMemory字幕背景` スクリプトが
`obj.ox += obj.w / 2` で右へずらして左揃えにする。**
その為、背景を切っていても (`BackOpacity=0`) スクリプトは貼る。
