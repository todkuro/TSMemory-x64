...
`Subtitle=1` のまま `[Caption] Enable=0` にしておく形をおすすめします。
**欲しくなった時に AviUtl2 の再起動だけで済み、視聴中の TVTest を
落とさずに切り替えられます。**

`Enable=0` の時は AviUtl2 側で字幕を一切読まないので、処理は増えません。
共有メモリに字幕のぶんが載りますが、**取り込む量の 0.01〜0.02%** です
(実測。放送 8 番組)。`MemorySize` で溜められる秒数はほぼ変わりません。
