...
放送は本文の無いデータユニット (CS = 画面消去だけ) で
「ここで消す」と送って来る。`CTSCaptionSource` は本文の無い
ユニットを捨てている為、**次の字幕が来るまで出しっぱなし**になる。

実測 (`test_caption` が字幕を持つ 4 番組で出す):

```
Record_20260827-013024.ts  放送 平均 3.9 秒 / 次まで 3.9 秒  6 組中 0 組
Record_20260827-013212.ts  放送 平均 4.3 秒 / 次まで 4.3 秒  3 組中 0 組
Record_20260827-110354.ts  放送 平均 3.0 秒 / 次まで 5.4 秒  1 組中 1 組 (差 2.4 秒)
Record_20260827-140438.ts  放送 平均 3.0 秒 / 次まで 3.0 秒  7 組中 0 組
```

**多くの局は消去を次の字幕と同じ時刻に送る**ので差が出ないが、
間が空く番組では長く出たままになる (17 組中 1 組、最大 2.4 秒)。
消去ユニットの PTS を終了時刻として持てば直る。
