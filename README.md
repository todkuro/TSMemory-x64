...
- 字幕の取り込みに対応しています。こちらも**両方**の設定が要ります。

      TSMemory.ini (TVTest 側)           TSMemory-TVTestSrc.ini (AviUtl2 側)
      [Settings]                         [Caption]
      Subtitle=1                         Enable=1

  字幕文ごとにテキストオブジェクトを作り、`[Caption] Layer` のレイヤーへ
  時間に合わせて並べます。外字 (DRCS) は放送されてきた字形をそのまま
  フォントに組み立てて使うので、代替文字になりません。

  書体をまとめて変えたい場合は、AviUtl2 側でテキストプリセットを作り、
  `[Caption] Preset` にその名前を書いてください。そのプリセットを直せば
  全ての字幕に効きます (タイムライン上で個別に触る必要はありません)。

  文字の表示位置と文字スーパーには対応していません。
