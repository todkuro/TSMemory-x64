...
**文字の大きさを変えた時の追従だけが違う。**TVCaptionMod2 は
文字を大きくすると縁も太くなるが、こちらは 2 ドットのまま。
`[Caption] UseBroadcastSize=0` でプリセットの大きさに任せる場合や
1080p 以外に出す場合は、トラックバー「縁取り」で合わせる。
...
if orn > 0 then
    ornok, ornerr = pcall(function()
        obj.effect("縁取り", "サイズ", orn, "縁色", orncol)
    end)
end
--  基準点が左上なので、四方に広がると文字が右下へ動く。
--  ただし本体側が既にずらしている場合もあるので、
--  obj.ox が動いていない時だけこちらで直す
...
| 符号 | 状態 | 実測 |
| --- | --- | --- |
| ORN (縁取りの色) | 取り出すが出力に使っていない | 8 本中 2 本。色は常に黒 |
| HLC (囲み) | 引数を捨てている | 未計測 |
| FLC / POL / WMM / CDC | 引数を捨てている | 未計測 |
| APF / APB / APU / PAPF | 送りに反映していない | **4 番組で 0 件** |

APF 系は実測で一度も来ていない。来た時に位置がずれるだけで
本文は壊れないが、対応するなら `State::PenX` を動かす所に足す。
