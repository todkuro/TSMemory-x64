...
1. DRCS のビットマップから TTF を組み立てる (1 画素 = 1 矩形の輪郭)
2. ARIB の 8 単位符号を復号する (**実装の本体**)
3. 字幕文の PTS からタイムライン上のフレーム範囲を求めて配置

> **リングバッファの窓に注意。**
> DRCS の定義は再送されるが、実測で**間隔の中央値が 14.2 秒**、
> 最大 210 秒。既定の `MemorySize=10` (8〜14 秒) とほぼ同じで、
> **字形の定義が窓に入らない事が半々程度で起こる**。
> DRCS の定義だけを別に保持し続ける (実測で 1 番組あたり 4 種 ×
> 324 バイト) のが解になる。
